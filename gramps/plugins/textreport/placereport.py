...
class PlaceReport(Report):
    ...
    def __init__(self, database, options, user):
        ...
        self.locale = self.set_locale(menu.get_option_by_name(
            'trans').get_value())
    ...
    def __write_place(self, handle, place_nbr):
        ...
        for level in get_location_list(self._db, place):
            ...
            place_details.append(_("%(str1)s: %(str2)s") %
                                 {'str1': level[1].str(self.locale),
                                  'str2': level[0]})
        ...
        for place_name in place.get_names():
            ...
            if not place_name.get_date_object().is_empty():
                place_names += ' [%s]' % self._get_date(
                    place_name.get_date_object())
        place_details.append(self._("places|All Names:") + place_names)
        place_types = ''
        for ptype in place.get_types():
            ...
            if not ptype.get_date_object().is_empty():
                place_types += ' [%s]' % self._get_date(
                    ptype.get_date_object())
        place_details.append(self._("places|All Types:") + place_types)

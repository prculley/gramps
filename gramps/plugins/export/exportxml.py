...
#        self.status = None
    ...
    def write_place_types(self):
        """ write place types and groups information.  We always include the
        string name, although it may have been changed or be in a different
        language.  For types that are numbered in a definitive and repeatable
        way, we also include the number.
        On import, if the user already has the type by number, the groups and
        name will be ignored, if he only has name, number and groups will be
        ignored, if he doeesn't have the type we will import it all.
        """
        self.g.write("  <place-types>\n")
        for number, tup in sorted(PlaceType.DATAMAP.items()):
            if number == PlaceType.CUSTOM:
                continue
            # if type is in numbered custom or standard range
            if number > PlaceType.CUSTOM:
                # the number is definitive and should be included
                numstr = ' number="%d"' % number
            else:
                numstr = ''
            if tup[1]:   # if a group is set
                # write out type/groups for this place type.
                self.g.write('    <place-type name="%s"%s>\n'
                             % (escxml(tup[0]), numstr))
                for gr_num, gr_nams in PlaceType.GROUPMAP.items():
                    if tup[1] & gr_num:
                        self.write_line("pgroup", escxml(gr_nams[1]), 3)
                self.g.write('    </place-type>\n')
            else:  # just write out the type
                self.g.write('    <place-type name="%s"%s/>\n'
                             % (escxml(tup[0]), numstr))
        self.g.write("  </place-types>\n")
    ...
    def dump_place_type(self, placetype, index=1):
        """ type, date, citations """
        _sp = "  " * index
        ptype = self.fix(placetype.xml_str())
        pnum = ''
        if placetype.is_numbered():
            pnum = ' number="%d"' % int(placetype)
        self.g.write('%s<ptype value="%s"%s' % (_sp, ptype, pnum))
    ...
    def dump_place_name(self, place_name, index=1):
        ...
        if(date.is_empty() and not place_name.get_citation_list() and not
           place_name.get_abbrevs()):
            self.g.write('/>\n')
        else:
            self.g.write('>\n')
            if not date.is_empty():

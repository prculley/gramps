...
from gramps.gen.lib import (
    ...
    PlaceRef, PlaceType, PlaceGroupType as P_G,
...
# Despite the name, the following are Gramps ID to handle
    ...
    def __add_place(self, event, sub_state):
        ...
        if sub_state.place:
            # we have a place
            ptypes = [] if (self.loc_loc_mode and sub_state.place.gramps_id)\
                ...
                if a_place is not None:
                    ...
                    if indx != 0:
                        ...
                        if a_place.group == P_G.NONE:
                            a_place.set_group(
                                a_place.get_type().get_probable_group())
            ...
            for name in name_list:
                if name.value == addendum.value:
                    if name.get_date_object().is_empty() and not name.lang:
                        ...
                        name.merge(addendum)
                    elif(addendum.get_date_object().is_empty() and
                         not addendum.lang):
                        name.merge(addendum)
                    else:
...
#            name = self.__parse_name_personal(line.data)
    ...
    def __date_cit_type(self, state, one_line=False):
        ...
        if not sub_state.event.get_date_object().is_empty():
            text += fmt % (_("Date:"),
                           complete_gedcom_date(
                               sub_state.event.get_date_object()))
    ...
    def __parse_obje(self, line):
        ...
        if value and value[0]:  # found from filename
            ...
            if '/' in state.form:  # already has expanded mime type
                ...
                value = mimetypes_types_map.get('.' + state.form,
                                                _('unknown'))
...
# FTM non-standard DATE in OBJE, treat as Media Date.
    ...
    def __build_event_pair(self, state, event_type, event_map, description):
        ...
        if(description == 'Y' and event.get_date_object().is_empty() and

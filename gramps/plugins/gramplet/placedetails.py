...
class PlaceDetails(Gramplet):
    ...
    def display_place(self, place):
        ...
        for name in place.get_names():
            names.append("%s (%s)" % (name.get_value(), name.get_language())
                         if name.get_language() else name.get_value())
            dates.append("" if name.get_date_object().is_empty() else
                         "[%s]" % name.get_date_object())
        ...
        for typ in place.get_types():
            ...
            dates.append("" if typ.get_date_object().is_empty() else
                         "[%s]" % typ.get_date_object())

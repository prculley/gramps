...
class Place(CitationBase, NoteBase, MediaBase, UrlBase, AttributeBase,
    ...
    def add_name(self, name, sort=True):
        ...
        if self.name_list and sort:
            if(name.lang == glocale.lang[:2] or not name.lang):
                if(self.name_list[0].lang == glocale.lang[:2] or
                   not self.name_list[0].lang):
                    # langs match, now compare dates
                    if(name.get_date_object().is_empty() and not
                       self.name_list[0].get_date_object().is_empty() or
                       not self.name_list[0].get_date_object().is_empty() and
                       self.name_list[0].get_date_object() <
    ...
    def add_type(self, p_type, sort=True):
        ...
        :type sort: bool
        """
        if(len(self.type_list) == 1 and
           self.type_list[0].pt_id == PlaceType.UNKNOWN):
            self.type_list[0] = p_type
            return
        if p_type in self.type_list:
            return
        if self.type_list and sort:
            if(p_type.get_date_object().is_empty() and not
               self.type_list[0].get_date_object().is_empty() or
               not self.type_list[0].get_date_object().is_empty() and
               self.type_list[0].get_date_object() < p_type.get_date_object()):
    ...
    def add_placeref(self, placeref, sort=True):
        ...
        for indx in range(len(self.placeref_list)):
            ...
            if(placeref.type == PlaceHierType.ADMIN and
               l_pref.type != PlaceHierType.ADMIN):
                break
            if(placeref.type != l_pref.type):
                continue
            if(not placeref.get_date_object().is_empty() and
               l_pref.get_date_object().is_empty()):
                break
            if(not l_pref.get_date_object().is_empty() and
               placeref.get_date_object() > l_pref.get_date_object()):
                break
        self.placeref_list = (self.placeref_list[:indx] + [placeref] +
                              self.placeref_list[indx:])

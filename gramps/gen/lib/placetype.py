...
#     NUMBER = "Number"       # 20
    ...
    DATAMAP = {
        ...
        CUSTOM :  ('', '',
                   "", "#0000FFFF0000", p_g(p_g.NONE), None),
    }

    str_to_pt_id = {}  # used for finding type from imported place type value
    ...
    def is_equivalent(self, other):
        ...
        :rtype: int
        """
        if(self.pt_id != other.pt_id or self.name != other.name or
    ...
    def is_empty(self):
        ...
        value)
        """
        return (self.pt_id == PlaceType.UNKNOWN and
    ...
    def update_name_map(cls):
        ...
        for (pt_id, tup) in cls.DATAMAP.items():
            cls.str_to_pt_id[tup[DM_NAME].lower()] = pt_id
        # and translated names, if they are not already there.
        for (pt_id, tup) in cls.DATAMAP.items():
            transfunc = tup[DM_TRANS]
            if transfunc:
                transname = transfunc(tup[DM_NAME], pt_id=pt_id).lower()
            else:
                transname = _(tup[DM_NAME]).lower()
            if transname not in cls.str_to_pt_id:
                cls.str_to_pt_id[transname] = pt_id

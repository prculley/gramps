...
def build_placetype_menu(uistate):
    """
    Create the menu items for these commands.
    """
    actions = []

    actions.append(('ReplacePlaceType', ReplacePlaceType))
    actions.append(('AssignPlaceGroup', AssignPlaceGroup))

    placetype_action = ActionGroup(name='PlaceType')
    placetype_action.add_actions(actions)
    uistate.uimanager.insert_action_group(placetype_action)
...
class ReplacePlaceType(ManagedWindow):
    ...
    def __init__(self, dbstate, uistate, track):
        ...
        self.o_ptype = place.get_placetype()
    ...
    def _save(self):
        ...
        pmon = progressdlg.ProgressMonitor(
            ...
            popup_time=2)
        status = progressdlg.LongOpStatus(msg=_("Replacing place types"),
...
class AssignPlaceGroup(ManagedWindow):
    ...
    def __init__(self, dbstate, uistate, track):
        ...
        self.place_group = MonitoredDataType(cbe,
                                             self.place.set_group,
                                             self.place.get_group,
    ...
    def _save(self):
        ...
        pmon = progressdlg.ProgressMonitor(
            ...
            ("", self.uistate.window, Gtk.DialogFlags.MODAL),
            popup_time=2)
        status = progressdlg.LongOpStatus(msg=_("Assigning place Groups"),
                                          total_steps=len(self.selected),
                                          interval=len(self.selected) // 20)
        pmon.add_op(status)
